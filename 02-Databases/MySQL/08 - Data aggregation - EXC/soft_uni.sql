...
#Task 12
SELECT `department_id`, MIN(`salary`) AS 'minium_salary'  FROM `employees`
...
AVG(`salary`) AS 'avg_salary' FROM hpe
GROUP BY `department_id`
ORDER BY `department_id`;
...
WHERE e.`salary` > (
	SELECT 
	AVG(inn.`salary`) FROM `employees` AS inn
	WHERE inn.`department_id` = e.`department_id`
	GROUP BY inn.`department_id`
)
