...
#Task 4
CREATE VIEW `prep` AS
SELECT b.`deposit_group`, 
AVG(b.`magic_wand_size`) AS `average_wand_size_per_group`
FROM `wizzard_deposits` AS b
GROUP BY b.`deposit_group`
ORDER BY `average_wand_size_per_group` ASC
LIMIT 1;

SELECT `deposit_group` FROM `prep`;
