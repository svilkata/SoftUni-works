...
public class Database {
    ...
    public void add(Person person) throws OperationNotSupportedException, ArrayIndexOutOfBoundsException {
        if (index + 1 == this.elements.length) { //когато стигнем 16тия елемент
        ...
        int newIDtoAdd = person.getId();
        for (Person element : this.elements) {
            if (element == null) {
                break;
            }
            if (element.getId() == newIDtoAdd) {
                throw new OperationNotSupportedException();
            }
        }
