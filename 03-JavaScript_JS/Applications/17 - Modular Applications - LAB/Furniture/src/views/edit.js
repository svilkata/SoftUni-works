...
export function editPage(ctx) {
    ...
    async function onSubmit(event) {
        ...
        //списък от полетата, които са празни. Като материала не е задължителен и може да е празен
        const missing = formData.filter(([inputKey, v]) => inputKey != 'material' && v.trim() == '');

        try {
            if (missing.length > 0) {
                const errors = missing.reduce((a, [k]) => Object.assign(a, {[k] : true}), {}); //непопълнените задължителни полета ги направи на обект от ключ и стойност
                console.log(errors);
                throw {
