...
export function catalogPage(ctx) {
    ...
    ctx.renderProp(catalogTemplate(loadItems(userpage), userpage));
}
...
async function loadItems(userpage) {
    ...
    if (userpage) {
        ...
        items = await getAll(); //масив от записи от сървъра
    }
    

    return items.map(itemTemplate); //масив от рендирани темплейти за всеки item
}
